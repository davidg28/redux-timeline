...
import { Grid, Button, DialogTitle, Dialog, TextField } from "@material-ui/core";
...
export default function UploadTimeline({ handleClose }) {
    ...
    const handleUploadData = async () => {
        ...
        try {
            ...
            const timeline_key = await firebase.database().ref(upload_refrence).push().getKey()
            const imageURL = await uploadImage(userImage, currentUser.uid, timeline_key)
            if (!imageURL) {
                toast.error("Error uploading image !")
                return
            }
            await firebase.database().ref(upload_refrence + '/' + timeline_key).set({
                username: username,
                date: date,
                time: time,
            ...
            user_data.forEach(async (item) => {
                ...
                await firebase.database().ref(data_upload + '/' + key).set({
                    id: key,
    ...
    return (
        ...
        <Dialog open={true} fullWidth maxWidth='sm' onClose={handleClose} aria-labelledby="form-dialog-title">
            ...
            <Grid container lg="12" xs="12" sm="12" md="12" justify="center">
                ...
                <Grid container lg="11" spacing="1" justify='center' style={{ marginTop: 20 }}>
                    {isUploading && <center><Dots /></center>}
                </Grid>
                <Grid container lg="11" spacing="1" justify='center' style={{ marginTop: 20, marginBottom: 20 }}>
