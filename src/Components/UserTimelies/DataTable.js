...
export default function DataTable({ setSelectedRows }) {
    ...
    const UpdateData = async (newData) => {
        const user_data_edit_ref = 'Users/' + currentUser.uid + '/Timelines/' + newData['timeline_id']
        try {
            firebase.database().ref(user_data_edit_ref).update(newData).then(() => {
    ...
    const DeleteData = async (deleteData) => {
        ...
        const user_data_delete_ref = 'Users/' + currentUser.uid + '/Timelines/' + deleteData['timeline_id']
        const data_delete_ref = 'Timelines/' + currentUser.uid + '/' + deleteData['timeline_id']

        try {

            await firebase.database().ref(user_data_delete_ref).remove()
            await firebase.database().ref(data_delete_ref).remove().then(() => {
